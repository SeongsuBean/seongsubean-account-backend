...
public class AccountRestController {
  ...
  public ResponseEntity<Map<String, Boolean>> checkPw(Authentication auth,
    ...
    return ResponseEntity.ok(Map.of("result", bCryptPasswordEncoder.matches(requestEditProfile.getPassword(), accountDetails.getUser().getEncryptedPwd())));
  }
  ...
  public ResponseEntity<ResponseProfile> getProfile(Authentication auth) {
    AccountDetails accountDetails = (AccountDetails) auth.getPrincipal();
    ResponseProfile profile = new ModelMapper().map(accountDetails.getUser(), ResponseProfile.class);
